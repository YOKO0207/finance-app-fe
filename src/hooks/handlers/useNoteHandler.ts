...
import {
	...
	useNoteDeleteFetcher,
	useNoteUpdateFetcher,
} from "@/hooks";
...
export const useNoteCreateHandler = () => {
	const { createNote, isFormLoading } = useNoteCreateFetcher();

	const handleNoteCreate = (args: { input: NoteInput }) => {
		const { input } = args;
...
export const useNoteDeleteHandler = () => {
	...
	const handleNoteDelete = (args: { noteId: string }) => {
		const { noteId } = args;
		const apiUrl = generateUrl(baseNoteApiUrl, { noteId });
		const validationErrors = deleteNote({
			apiUrl,
			mutateApiUrls: [baseNotesApiUrl],
		});
